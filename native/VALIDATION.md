...
**Source48 checkpoint, 12 September 2026:** all 30 native suites, all 44 fixed requests, 12 saved replay/convergence cases, eight independent organic audits, fresh portable checks and ten real UE contracts pass. All 12 saved rides/plans/traces remain identical to source47. [Current qualification](V083_INTEGRATION.md) records exact receipts and hashes. The latest user-requested quiet-running review confirms an unresolved design issue in unpowered connectors; passing physical acceptance does not complete that intent. No screenshot-derived threshold has been introduced. Final-head three-platform CI and promotion remain pending. Earlier dated results below are separate checkpoints.

**Current source47 evidence, 12 September 2026:** [the qualification report](V083_INTEGRATION.md) records exact results and hashes for pushed implementation `4906de0`. All 30 native suites pass on one frozen build; Python and fresh portable checks pass. The required eight plus four added regressions accept candidate zero, replay exactly and pass independent 960/1920 Hz audits. All eight required organic audits pass; hills2 retains its distant transverse crossing and 16.4777% straight-flat share. The predeclared 44-request matrix and three-platform CI are running. Final-commit qualification and main CI are not yet complete. [Current progress](V083_PROGRESS.md).

The following dated results are retained historical checkpoints, not final qualification of the current source.

**Current rewrite evidence, 2026-09-12:** [the progress checklist](V083_PROGRESS.md) records the active seven-step goal. `integration-20260911/itinerary-physical-crossings-21` accepted flat5, flat42, hills2, hills9 and canyon24 on candidate zero, with exact saved replay and complete160-metric 960/1920 Hz audit; flat7 and both remaining canyon fixtures failed. The hills2 save independently passes6,358 organic checks, with one distant transverse crossing,70.5m vertical separation and9.7% straight-flat share. Baseline and circuit components passed56 and4,104 checks. Build37's `itinerary-partial-feedback-25` separately accepts canyon42 after seven rebuilds and a0.47507m/s residual; canyon1 times out at600s. These snapshots cannot be combined into a final green receipt. Inlet-bound and rigid-turn experiments remain rejected and preserved. The fixed-source ownership study is running. The44-request matrix, complete final native/Python/portable checks, final integration CI and merge remain pending. The older mixed-build `itinerary-suite-9.log` (22/29 passing) is diagnostic only.

**Later isolated architecture evidence:** `itinerary-fixed-footprint-31` accepts five of eight (flat5, flat7, flat42, hills2, canyon1); all five have exact saved replay and independent960/1920 audits. Its hills2 save passes6,358 organic checks, with one distant crossing,102.45m separation, four crest shapes and17.68% straight-flat share. `full-suite-31/ctest.log` is21/29 in912.18s; all eight failures remain recorded. Python discovery is176 passed and8 skipped (184 discovered); all eight real CLI tests pass separately in that CTest run. `solver-diagnosis-4/` independently proves a feasible opposite crossing order that the former sign guess rejected. Build3's sampling correction passes5,373 circuit checks and accepts hills9 with exact replay/audit in `itinerary-sampling-32` (183.111458s, save SHA-256 `c21afc8731c4aca24897445c9748a12df6fcdfc062ab8696a1b2c2507139b76e`). Build4 additionally addresses terrain-driven footprint sizing and connector onset; its57 baseline and180 historical force checks pass, while all eight rides are under test in `itinerary-terrain-footprint-33`. These remain isolated, unpromoted source snapshots.

**Architecture review, 2026-09-11:** the uncommitted common5/booster experiment now passes all 29 local native CTest suites and all eight requested candidate-zero saves, independent exact physical replays and 960/1920 audits. Source/executable/save hashes and failed attempts are preserved in `artifacts/flow-intent-v083-20260910/integration-20260911/`. The user has redirected the work from fixture-driven route tuning to a simpler general generator. These passing experiments are not promoted evidence for that redesign. No final integration commit or three-platform CI/merge has occurred. See [the architecture review and precise scope](GENERATOR_ARCHITECTURE.md). Earlier records below remain historical.

**Verified intermediate repairs:** local commit `83f5f35` repairs current-CLI acceptance and the full160-metric replay contract;176 Python tests, eight real CLI tests and all eight preserved saves pass. [Protocol receipts](artifacts/flow-intent-v083-20260910/integration-20260911/protocol-review/REVIEW.md). Shared canonical source placement, including the loop and Immelmann, passes source-component and full organic-generation checks. [Source receipts](artifacts/flow-intent-v083-20260910/integration-20260911/source-geometry-review/REVIEW.md). These checks validate those changes, not completion of the still-unfinished itinerary/route redesign.
