...
void AVibeCoasterController::ChangeRow(int32 Direction)
{
    ...
    case 1: Settings.terrain.kind = static_cast<coaster::TerrainKind>((static_cast<int32>(Settings.terrain.kind) + Direction + 3) % 3); break;
    case 2: Settings.targets.requireIntensity = !Settings.targets.requireIntensity; break;
    case 3: Settings.targets.height = FMath::Clamp(Settings.targets.height + Direction * 5, 220., 350.); break;
    case 4: Settings.targets.speed = FMath::Clamp(Settings.targets.speed + Direction, 75., 110.); break;
    case 5: Settings.targets.inversionHeight = FMath::Clamp(Settings.targets.inversionHeight + Direction * 5, 80., 140.); break;
    case 6: Settings.targets.launchSeconds = FMath::Clamp(Settings.targets.launchSeconds + Direction * .05, .8, 1.4); break;
    case 7: Settings.maxCandidates = FMath::Clamp(Settings.maxCandidates + Direction, 1, 32); break;
    }
}
...
void AVibeCoasterController::PlayerTick(float DeltaSeconds)
{
    ...
    if (WasInputKeyJustPressed(EKeys::Up)) SelectedRow = (SelectedRow + 7) % 8;
    if (WasInputKeyJustPressed(EKeys::Down)) SelectedRow = (SelectedRow + 1) % 8;
...
FString AVibeCoasterController::RowText(int32 Row) const
{
    ...
    case 5: return FString::Printf(TEXT("Inversion height above ground >= %.0f m"), Settings.targets.inversionHeight);
    case 6: return FString::Printf(TEXT("Launch 0-180 km/h <= %.2f s"), Settings.targets.launchSeconds);
    case 7: return FString::Printf(TEXT("Candidate search budget: %d"), Settings.maxCandidates);
...
void AVibeCoasterHUD::DrawHUD()
{
    ...
    if (PC->Menu)
    {
        ...
        Y += 8 * Scale;
        for (int32 Row = 0; Row < 8; ++Row) Line((Row == PC->SelectedRow ? TEXT("> ") : TEXT("  ")) + PC->RowText(Row), Row == PC->SelectedRow ? Accent : FLinearColor::White);
        Y += 8 * Scale;
