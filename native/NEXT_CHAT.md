**Current continuation: placed-track ownership + CI qualification is pending.** Read the new section at the top of [V083_INTEGRATION.md](V083_INTEGRATION.md) and [the current plan/evidence](artifacts/native-recovery-20260912/PLAN.md). Native simulations run on CI. Local Unreal repair/checks and other high-load work are paused while the user is gaming. Finish code/CI checks, then let the user verify before merging. Preserve unrelated Conventional1 art and all historical evidence.

**Portable-test correction:** the deflation/skeptic loop found that portable `-Test` omitted the eight real CLI contracts. The direct invocation and its Windows PowerShell 5.1 compatibility fix passed scoped review, and all eight contracts passed against the fresh executable. Read [qualification](V083_INTEGRATION.md#portable-driver-review-after-source64), the [current ledger](artifacts/deflation-skeptic-20260912/progress.md) and [promotion receipts](artifacts/deflation-skeptic-20260912/promotion/). A new `complete.json` is written only after the full local driver, exact-commit Windows/macOS integration CI, merge and main CI pass. The source64 record below is the completed baseline, not qualification of the new driver. The user's latest instruction is normal independent workflow with fewer agents.
...
2. Any subsequent native correction needs a rebuilt, consistently hashed qualification. Retain all 36 suites, all 44 unchanged requests, the required eight rides, ten UE contracts and independent audits. Never substitute historical passes or seeds. A tooling-only correction needs fresh verification of the changed tooling and explicit binding to unchanged native inputs; it creates no new physical or UE runtime evidence.
...
Tool paths and focused/full commands are in [BUILDING.md](tools/BUILDING.md). VS uses `Visual Studio 17 2022`, x64, `v143,version=14.38`, instance `D:/Toolchains/VS2022`, SDK `10.0.22621.0`. Current Python is `C:/Users/danie/AppData/Local/Programs/Python/Python313/python.exe`, used with `-B` and matplotlib 3.11.1. The UE 5.8 directory currently lacks its editor and build tools; [the other task's investigation](artifacts/deflation-skeptic-20260912/unreal-investigation.json) records payload loss around the Launcher migration, with the exact cause unproven and no backup found. Historical source64 UE results remain tied to their original engine, binaries and saves.
