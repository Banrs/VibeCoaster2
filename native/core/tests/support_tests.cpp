...
int main(){try{
    ...
    auto d=baseline;d.station=buildStation(d.track,d.request.terrain,d.request.train);buildSupportLayout(d);d.generationVersion=generatorVersion;d.report=validateGeometry(d.track,d.request.terrain,d.request.limits,d.request.train,d.supports);d.simulation=simulate(d.track,d.operations,d.request.train,d.request.simulationStep);evaluateTargets(d);assessAuthorship(d);assessMotion(d);verifyConvergence(d);check(d.accepted(),"Explicit tower replacement accepted");check(validateDesignStructures(d).valid(),"Explicit replacement station and supports mutually clear");check(d.track.knots.size()==baseline.track.knots.size(),"Support replacement preserves knot count");
