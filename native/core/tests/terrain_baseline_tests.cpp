...
int main(){try{
    ...
    auto tied=solveTerrainBaseline(distance,floor,target,authored,{{0,0,0,false,1,true},{400,400,0,false,1,true}},{{0,400,stations(400),-1,1}});
    check(std::abs(tied.height(0)-.7)<1e-10&&tied.height(0)==tied.height(400),"A transfer with one shared station variable retains the correct quadratic objective");
    std::fill(floor.begin(),floor.end(),-1);floor[200]=8;
    std::fill(target.begin(),target.end(),0);
    BaselineDomain transfer{100,300,stations(200),-10,10};
    auto powered=solveTerrainBaseline(distance,floor,target,authored,{{0,20,0,true},{380,400,0,true}},{transfer});joins(powered);ordinaryFloor(powered,distance,floor);
    check(powered.transfers.size()==1,"The declared motor retains one unwarped S7 owner");
    const auto& profile=powered.transfers[0].profile;double transferDeficit=-INFINITY;
    for(size_t i=0;i+1<transfer.horizontal.size();++i)for(int j=0;j<=20;++j){const double u=j/20.,at=i+u;transferDeficit=std::max(transferDeficit,floor[100+i]+u*(floor[101+i]-floor[100+i])-profile.height(at));}
    check(transferDeficit<1e-8,"Motor ridge clearance constrains its solved source heights continuously");
    check(profile.finish-profile.start>=-10-1e-8&&profile.finish-profile.start<=10+1e-8,"Motor rise remains inside its supplied physical interval");
    const TerrainMotion motion{45,.0196133,.000163333333333,-2.8,3.5,65,13};
    check(terrainMotionFits(assessTerrainMotion(profile,motion,{},4096),motion),"Independent loss-aware motion accepts the actual solved motor profile");
    auto rampAuthored=authored;for(size_t i=0;i<rampAuthored.size();++i)rampAuthored[i]=10+distance[i]/20;
    auto rampFloor=floor;for(size_t i=0;i<rampFloor.size();++i)rampFloor[i]-=rampAuthored[i];
    auto absolute=solveTerrainBaseline(distance,rampFloor,target,rampAuthored,{{0,20,0,true},{380,400,0,true}},{transfer});
    const auto& absoluteProfile=absolute.transfers[0].profile;
    check(absoluteProfile.start==absolute.height(100)+rampAuthored[100]&&absoluteProfile.finish==absolute.height(300)+rampAuthored[300],"Transfer endpoints retain their authored absolute-height offsets");
    check(absoluteProfile.finish-absoluteProfile.start<=10+1e-8,"Rise limits constrain absolute height, not only source offsets");
    distance=stations(500);floor.assign(501,-10);target.assign(501,0);authored.assign(501,0);
    const BaselineDomain supplyTransfer{200,450,stations(250),-50,50};
    for(int cars:{6,12}){
        TrainConfig train;train.cars=cars;
        const auto supplied=solveTerrainBaseline(distance,floor,target,authored,{{0,50,30,true},{450,500,0,false}},{supplyTransfer},{},{},{},{},{{50,200,65,0,60}},train);
        std::vector<AuthoredPoint> points;for(int i=0;i<=500;++i){
            const double height=i>=200&&i<=450?supplied.transfers.front().profile.height(i-200):supplied.height(i);
    ...
    floor.assign(501,-1);target.assign(501,0);
    ...
    // Independent two-port optimum: symmetry fixes the mean height at 5;
    // the rise constraint is active because the linear target rises by 10.
    auto analyticDistance=stations(400);std::vector<double> analyticFloor(401,-100),analyticTarget(401),analyticAuthored(401);
    for(size_t i=0;i<analyticTarget.size();++i)analyticTarget[i]=double(i)/40;
    const auto analytic=solveTerrainBaseline(analyticDistance,analyticFloor,analyticTarget,analyticAuthored,{{0,0,-100,false},{400,400,-100,false}},{{0,400,analyticDistance,-2,2}});
    check(std::abs(analytic.height(0)-4)<1e-8&&std::abs(analytic.height(400)-6)<1e-8,"Independent analytic constrained transfer optimum matches the joint QP");
    BaselineDomain endpointWindow{0,400,analyticDistance,-2,2,100,300+4e-12};
    const auto roundedWindow=solveTerrainBaseline(analyticDistance,analyticFloor,analyticTarget,analyticAuthored,{{0,0,-100,false},{400,400,-100,false}},{endpointWindow});
    check(roundedWindow.transfers[0].profile.activeStart==endpointWindow.activeStart&&roundedWindow.transfers[0].profile.activeLength==endpointWindow.activeLength,
        "World-chord endpoint roundoff preserves the exact certified active window");
    endpointWindow.activeLength=300.0001;bool outsideWindow=false;
    try{solveTerrainBaseline(analyticDistance,analyticFloor,analyticTarget,analyticAuthored,{{0,0,-100,false},{400,400,-100,false}},{endpointWindow});}
    catch(const std::invalid_argument&){outsideWindow=true;}
    check(outsideWindow,"A materially out-of-domain window still rejects instead of being shortened");
    // A floor near a tightly bounded port gives almost parallel constraints,
    // but its far port can still move: this is feasible, not a rank failure.
    const std::vector<double> shortPortDistance{0,4,400},shortPortFloor{-1,1e-5,-1},zero(3);
    const auto shortPort=solveTerrainBaseline(shortPortDistance,shortPortFloor,zero,zero,{{0,0,0,false,0},{2,2,0,false}},{{0,2,shortPortDistance,-100,100}});
    const TerrainTransfer unit{0,1,400};
    check(std::abs(shortPort.height(0))<1e-8&&std::abs(shortPort.height(400)-1e-5/unit.height(4))<1e-4,"A feasible almost-parallel port constraint retains its independent analytic height");
    ...
    try{solveTerrainBaseline(analyticDistance,analyticFloor,analyticTarget,analyticAuthored,{{0,0,-100,false},{400,400,-100,false}},{{0,400,analyticDistance,-2,2}},{},{},[&]{return ++cancellationPolls==3;});}
    catch(const std::exception& error){interrupted=std::string(error.what())=="CANCELLED";}
    check(interrupted,"Cancellation is observed during a transfer-only solve, not only before it");
