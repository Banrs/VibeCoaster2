...
The unused `buildReversingModule()` and `buildEnergyReversingModule()` authoring adapters were retired in 0.8.2. Their prescribed half-loop/straight-roll families had no game callers. The generator uses `designFvdReversal()` and its descending pullout, documented below. This removes obsolete source APIs and their request/result types; canonical save geometry and operations are unchanged by this cleanup.
...
The generator supplies the requested train's explicit loss coefficients and searches source entry speed from `max(75, requestedSpeed)` through 90 m/s in 0.25 m/s increments. It requires a minimum source traversal speed of 25 m/s, an explicit pacing intention rather than a standard or promised final speed. The physical departure motor targets the selected source inlet plus 0.5 m/s; actual force, power and exit fade still determine acceleration. Both measured source inlet and outlet must agree within 0.5 m/s, or `AUTHORING_SIGNATURE` rejects the ride.

The solved source is immutable and reused within one candidate's feedback loop. Its actual horizontal span and outlet pose feed route closure. Terrain may translate the whole source to the station datum, within the existing 16 m station bay budget; it cannot bend the source to follow a rim. Exact source tangent, curvature and up vectors are restored after canonical composition, and nonrigid displacement is rejected. Canyon sites are ranked along the actual valley tangent. The following terrain climb ends at the inversion source inlet, so its pitch can unwind through the existing trim approach without imposing a level reset at the motor/brake boundary.

The selected seeded height must be within 220–280 m and the requested minimum speed at most 90 m/s. Unsupported source-family requests terminate explicitly with `SOURCE_FAMILY`, preserving their targets and leaving the wider general request/persistence domain intact. There is no geometric fallback or target clamp. Broader signature families remain future work. Eleven targeted complete circuits passed exact saved replay and independent convergence; [integration evidence and limits](../artifacts/generator-intent-v081/tall-hill-integration-v1/REPORT.md) distinguish this from population acceptance and whole-ride review.

## Whole-route energy feedback and its limits

The generator permits at most eight feedback rebuilds at the selected route family/site. It compares the tall signature's actual inlet/outlet speeds, four airtime entry speeds, the full-loop apex speed and the reversal exit speed with their authoring intentions. The maximum speed residual must be at most 0.5 m/s; otherwise `AUTHORING_ENERGY` rejects the candidate, with the additional signature-specific gate above. This replaces the previous single correction. The reversal exit target applies after the roll, and should not be mistaken for an independently measured inverted-apex speed.
