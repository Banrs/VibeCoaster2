...
template<class T>inline std::array<T,8> flowFramePolynomial(T a,T aS,T aSS,T aSSS,T b,T bS,T bSS,T bSSS,double span){
    T c1=aS*span,c2=aSS*(span*span*.5),c3=aSSS*(span*span*span/6),p=b-a-c1-c2-c3,v=bS*span-c1-c2*2-c3*3,acc=bSS*(span*span)-c2*2-c3*6,j=bSSS*(span*span*span)-c3*6;
    return {a,c1,c2,c3,p*35-v*15+acc*2.5-j/6,p*(-84)+v*39-acc*7+j*.5,p*70-v*34+acc*6.5-j*.5,p*(-20)+v*10-acc*2+j/6};
}
...
inline void blendAuthoredJoin(std::vector<AuthoredPoint>& points,size_t first,size_t last,Cancel cancel={}){
    ...
    auto bankJet=[&](size_t index){const auto& p=source.spans[index];double q=norm(p.c[1]),qu=dot(p.c[1],p.c[2]*2)/q;
        const double quu=(4*dot(p.c[2],p.c[2])+6*dot(p.c[1],p.c[3])-qu*qu)/q;
        return std::array<double,4>{p.bank[0],p.bank[1]/q,2*p.bank[2]/(q*q)-p.bank[1]*qu/(q*q*q),6*p.bank[3]/(q*q*q)-6*p.bank[2]*qu/std::pow(q,4)-p.bank[1]*quu/std::pow(q,4)+3*p.bank[1]*qu*qu/std::pow(q,5)};};
    const auto ba=bankJet(4),bb=bankJet(right);
    const auto bank=flowFramePolynomial(ba[0],ba[1],ba[2],ba[3],bb[0],bb[1],bb[2],bb[3],span);
    ...
    const auto up=flowFramePolynomial(a.sample.up,a.upS,a.upSS,a.upSSS,b.sample.up,b.upS,b.upSS,b.upSSS,span);
