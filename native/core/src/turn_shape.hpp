...
struct TurnShape {
    ...
    double curvatureAt(double s) const{
        ...
        return peakBank>0?std::tan(peakBank*u)/(radius*std::tan(peakBank)):u/radius;
    }
};
inline TurnShape makeTurn(double angle,double radius,double ramp,double peakBank=0){
    ...
    TurnShape t;t.angle=angle;t.radius=radius;t.ramp=std::min(ramp,.85*radius*magnitude);t.length=radius*magnitude+t.ramp;t.peakBank=peakBank;
    if(peakBank==0){
