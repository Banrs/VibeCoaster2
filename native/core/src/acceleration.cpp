...
struct LimitPoint { double seconds; double magnitudeG; };
struct Curve {
    std::span<const LimitPoint> points;
    double limit(double seconds) const {
        if (seconds <= points.front().seconds) return points.front().magnitudeG;
        for (std::size_t i = 1; i < points.size(); ++i) if (seconds <= points[i].seconds) {
...
constexpr std::array xPositive{LimitPoint{.2, 6}, LimitPoint{1, 6}, LimitPoint{2, 4},
    LimitPoint{4, 4}, LimitPoint{5, 3}, LimitPoint{11.8, 3}, LimitPoint{12, 2.5}};
...
constexpr std::array zPositive{LimitPoint{.2, 6}, LimitPoint{1, 6}, LimitPoint{2, 4},
    LimitPoint{4, 4}, LimitPoint{5, 3}, LimitPoint{11.8, 3}, LimitPoint{12, 2}};
constexpr std::array zReduced{LimitPoint{.2, 5}, LimitPoint{1.5, 5}, LimitPoint{2, 4}, LimitPoint{2.5, 2}};
constexpr std::array zNegative{LimitPoint{.2, 2}, LimitPoint{.5, 1.5}, LimitPoint{4, 1.5}, LimitPoint{7, 1.1}};
...
bool scanRuns(std::span<const double> values, double step, int sign, double level,
    ...
    for (std::size_t i = first; i + 1 < values.size(); ++i) {
        ...
        auto peak = [&](double v, double time) {
            if (v > run.peak) { run.peak = v; run.firstPeak = time; run.lastPeak = time; }
            else if (v == run.peak) run.lastPeak = time;
        };
        if (inA) peak(a, t);
        if (inB) peak(b, last);
        run.end = end;
        if (!inB) { if (run.end > run.begin) callback(run); active = false; }
    }
    if (active && run.end > run.begin) callback(run);
    return true;
}
...
struct Biquad {
    ...
    Biquad(double step, double q, double equilibrium) {
        ...
        b0 = k * k * normalization; b1 = 2 * b0; b2 = b0;
...
bool measure(std::span<const double> values, double step, double start,
             AccelerationExtrema& extrema, AccelerationOnset& onset,
             const std::function<bool()>& cancel) {
    extrema = {values.front(), values.front(), start, start};
    for (std::size_t i = 0; i < values.size(); ++i) {
        if ((i & 4095) == 0 && cancel && cancel()) return false;
        const double time = start + static_cast<double>(i) * step;
        if (values[i] < extrema.minimumG) { extrema.minimumG = values[i]; extrema.minimumTimeSeconds = time; }
        if (values[i] > extrema.maximumG) { extrema.maximumG = values[i]; extrema.maximumTimeSeconds = time; }
    }
    ...
    if (halfSteps > static_cast<double>(values.size())) return true;
    ...
    if (edge >= values.size() || edge > (values.size() - 1) / 2) return true;
    ...
    for (std::size_t center = edge; center + edge < values.size(); ++center) {
        if ((center & 4095) == 0 && cancel && cancel()) return false;
        ...
        if (!onset.available || slope < onset.minimumGps) { onset.minimumGps = slope; onset.minimumTimeSeconds = time; }
        if (!onset.available || slope > onset.maximumGps) { onset.maximumGps = slope; onset.maximumTimeSeconds = time; }
...
void diagnose(AccelerationAssessment& result, const AccelerationSeries& series,
              const char* clause, const char* rule, std::string axis, std::string sign,
              double begin, double end, double actual, double limit, bool minimum = false) {
    ...
    result.diagnostics.push_back({clause, rule, std::move(axis), std::move(sign), series.seatIndex,
        series.startTimeSeconds + begin, series.startTimeSeconds + end, actual, limit, utilization});
}

bool validate(const AccelerationSeries& series, AccelerationAssessment& result,
              const std::function<bool()>& cancel) {
    if (cancel && cancel()) { result.cancelled = true; return false; }
    ...
    for (const auto values : axes(series)) for (std::size_t i = 0; i < size; ++i) {
        if ((i & 4095) == 0 && cancel && cancel()) { result.cancelled = true; return false; }
        if (!std::isfinite(values[i])) {
            diagnose(result, series, "7.1.1.2", "nonfinite-acceleration", "input", "", i * series.stepSeconds, i * series.stepSeconds, 0, 0);
            return false;
        }
    }
    return true;
...
const Curves baseCurves{std::array{Curve{xPositive}, Curve{xNegative}},
    std::array{Curve{yEither}, Curve{yEither}}, std::array{Curve{zPositive}, Curve{zNegative}}};
struct TimeWindow { double begin; double end; };

bool assessAxis(const AccelerationSeries& series, AccelerationAssessment& result,
                std::size_t axis, int sign, Curve curve, const char* clause, const char* rule,
                const std::function<bool()>& cancel, double from = 0,
    ...
    for (int i = 1; i <= static_cast<int>(std::floor(top * 10)); ++i) levels.push_back(i * .1);
    levels.push_back(top);
    for (const auto point : curve.points) if (point.magnitudeG <= top) levels.push_back(point.magnitudeG);
    ...
    for (const double level : levels) {
        if (!scanRuns(values, series.stepSeconds, sign, level, false, cancel, [&](const Run& run) {
            ...
            if (exceeds(level, limit)) diagnose(result, series, clause, rule, axisNames[axis], signName,
                run.begin, run.end, level, limit);
        }, from, to)) return false;
    }
    if (exceeds(maximum, cap) && !scanRuns(values, series.stepSeconds, sign, cap, true, cancel, [&](const Run& run) {
        if (!exceeds(run.peak, cap)) return;
        const bool sustained = atLeast(run.end - run.begin, sustainedSeconds);
        diagnose(result, series, sustained ? clause : "7.1.4.2", sustained ? rule : "impact-review-required",
            axisNames[axis], signName, run.begin, run.end, run.peak, cap);
    }, from, to)) return false;
    return true;
}
...
bool assessHistory(const AccelerationSeries& series, AccelerationAssessment& result,
    ...
    if (!scanRuns(values, series.stepSeconds, -1, 0, true, cancel, [&](const Run& run) {
        if (run.end - run.begin > 3 + timeRoundoff) negative.push_back(run);
    })) return false;
    if (!scanRuns(values, series.stepSeconds, -1, 0, false, cancel, [&](const Run& run) {
        if (atLeast(run.end - run.begin, sustainedSeconds)) weightless.push_back(run);
    })) return false;
    ...
    if (!scanRuns(values, series.stepSeconds, 1, 0, true, cancel, [&](const Run& run) {
        ...
        while (negativeIndex < negative.size() && negative[negativeIndex].end <= run.begin + timeRoundoff) {
            historyTrigger = true; ++negativeIndex;
        }
        if (historyTrigger) {
            const double until = std::min(end, run.begin + 6);
            if (!windows.empty() && run.begin <= windows.back().end) windows.back().end = until;
            else windows.push_back({run.begin, until});
        }
        while (weightlessIndex < weightless.size() && weightless[weightlessIndex].end <= run.begin + timeRoundoff) {
            qualifiedTransition = true; ++weightlessIndex;
        }
        if (!qualifiedTransition || run.peak < 2 || !completed) return;
        const auto first = static_cast<std::size_t>(std::max(0., std::floor(run.begin / series.stepSeconds)));
        for (std::size_t i = first; i + 1 < values.size() && i * series.stepSeconds < run.end; ++i) {
            if ((i & 4095) == 0 && cancel && cancel()) { completed = false; return; }
            if (values[i] < 2 && values[i + 1] >= 2) {
                const double crossing = (static_cast<double>(i) + (2 - values[i]) / (values[i + 1] - values[i])) * series.stepSeconds;
                if (!atLeast(crossing - run.begin, .133)) diagnose(result, series, "7.1.7.2", "weightless-to-positive-transition",
                    "Z", "+", run.begin, crossing, crossing - run.begin, .133, true);
...
bool assessReversals(const AccelerationSeries& series, AccelerationAssessment& result,
                     std::size_t axis, const std::function<bool()>& cancel) {
    std::array<std::vector<Run>, 2> runs;
    const auto values = axes(series)[axis];
    for (int side = 0; side < 2; ++side) if (!scanRuns(values, series.stepSeconds, side == 0 ? 1 : -1, 0, true,
        cancel, [&](const Run& run) { if (atLeast(run.end - run.begin, sustainedSeconds)) runs[side].push_back(run); })) return false;
    std::array<std::size_t, 2> next{};
    Run previous{}; int previousSide = -1;
    while (next[0] < runs[0].size() || next[1] < runs[1].size()) {
        if (cancel && cancel()) return false;
        const int side = next[1] == runs[1].size() ? 0 : next[0] == runs[0].size() ? 1 :
            runs[0][next[0]].begin < runs[1][next[1]].begin ? 0 : 1;
        const Run current = runs[side][next[side]++];
        if (previousSide >= 0 && side != previousSide && current.firstPeak - previous.lastPeak <= sustainedSeconds + timeRoundoff) {
            ...
            for (const auto entry : {std::pair{previous, previousSide}, std::pair{current, side}}) {
                ...
                if (exceeds(entry.first.peak, limit)) diagnose(result, series, "7.1.6.1", "rapid-reversal-peak",
                    axisNames[axis], entry.second == 0 ? "+" : "-", previous.lastPeak, current.firstPeak, entry.first.peak, limit);
            }
        }
...
bool assessLongDurationScope(const AccelerationSeries& series, AccelerationAssessment& result,
    ...
    for (std::size_t axis = 0; axis < 3; ++axis) for (int sign : {1, -1}) {
        ...
        if (!scanRuns(axes(series)[axis], series.stepSeconds, sign, threshold, true, cancel, [&](const Run& run) {
            if (run.end - run.begin > maximum + timeRoundoff) diagnose(result, series, "7.1.4.6",
                positiveZ ? "positive-tail-review-required" : "duration-over-90s-review-required", axisNames[axis], sign > 0 ? "+" : "-",
                run.begin, run.end, run.end - run.begin, maximum);
        })) return false;
    }
    return true;
}
...
bool assessPair(const AccelerationSeries& series, AccelerationAssessment& result, std::size_t firstAxis,
    ...
    auto close = [&] {
        if (active && atLeast(run.end - run.begin, sustainedSeconds)) diagnose(result, series, "7.1.5.1",
            peakReduced ? "combined-ellipse-negative-history" : "combined-ellipse", axis, peakSign, run.begin, run.end, run.peak, 1);
        active = false;
    };
    ...
    for (std::size_t i = 0; i + 1 < x.size(); ++i) {
        ...
        while (windowIndex < windows.size() && windows[windowIndex].end <= time) ++windowIndex;
        std::array<double, 8> cuts{};
        std::size_t cutCount = 0;
        cuts[cutCount++] = 0; cuts[cutCount++] = 1;
        for (const auto values : {x, y}) if ((values[i] < 0 && values[i + 1] > 0) || (values[i] > 0 && values[i + 1] < 0))
            cuts[cutCount++] = -values[i] / (values[i + 1] - values[i]);
        if (secondAxis == 2) for (std::size_t w = windowIndex; w < windows.size() && windows[w].begin < time + series.stepSeconds; ++w) {
            for (double boundary : {windows[w].begin, windows[w].end}) if (boundary > time && boundary < time + series.stepSeconds)
                cuts[cutCount++] = (boundary - time) / series.stepSeconds;
        }
        std::sort(cuts.begin(), cuts.begin() + cutCount);
        cutCount = static_cast<std::size_t>(std::unique(cuts.begin(), cuts.begin() + cutCount) - cuts.begin());
        for (std::size_t part = 1; part < cutCount; ++part) {
            ...
            bool reduced = false;
            if (secondAxis == 2 && midY > 0) for (std::size_t w = windowIndex; w < windows.size() && windows[w].begin <= midTime; ++w)
            ...
            auto root = [&](double r) { if (r > 0 && r < 1) boundaries[count++] = r; };
            if (qa > 0) {
                const double discriminant = qb * qb - 4 * qa * qc;
                if (discriminant >= 0) {
                    const double q = -.5 * (qb + std::copysign(std::sqrt(discriminant), qb));
                    if (q != 0) { root(q / qa); root(qc / q); }
                    else root(-qb / (2 * qa));
                }
            } else if (qb != 0) root(-qc / qb);
            std::sort(boundaries.begin(), boundaries.begin() + count);
            count = static_cast<std::size_t>(std::unique(boundaries.begin(), boundaries.begin() + count) - boundaries.begin());
            for (std::size_t sub = 1; sub < count; ++sub) {
                ...
                if ((qa * mid + qb) * mid + qc <= valueRoundoff) { close(); continue; }
                if (!active) { run = {begin, end, 0, begin, begin}; active = true; }
                run.end = end;
...
void finish(AccelerationAssessment& result) {
    std::sort(result.diagnostics.begin(), result.diagnostics.end(), [](const auto& a, const auto& b) {
        return std::tie(a.clause, a.rule, a.axis, a.sign, a.seatIndex, a.startTimeSeconds, a.endTimeSeconds, a.actual, a.limit) <
               std::tie(b.clause, b.rule, b.axis, b.sign, b.seatIndex, b.startTimeSeconds, b.endTimeSeconds, b.actual, b.limit);
    });
...
AccelerationAssessment assessProcessedAccelerationF2291_25(const AccelerationSeries& series,
    ...
    if (!validate(series, result, cancel)) return result;
    auto cancelled = [&] { result.cancelled = true; finish(result); return result; };
    for (std::size_t axis = 0; axis < 3; ++axis) if (!measure(axes(series)[axis], series.stepSeconds, series.startTimeSeconds,
        result.filteredExtrema[axis], result.onset100ms[axis], cancel)) return cancelled();
    std::vector<TimeWindow> history;
    if (!assessHistory(series, result, history, cancel)) return cancelled();
    for (std::size_t axis = 0; axis < 3; ++axis) for (int side = 0; side < 2; ++side) if (!assessAxis(series, result, axis,
        side == 0 ? 1 : -1, baseCurves[axis][side], "7.1.4", "sustained-duration", cancel)) return cancelled();
    for (const auto window : history) if (!assessAxis(series, result, 2, 1, {zReduced}, "7.1.7.1", "negative-history-duration",
        cancel, window.begin, window.end)) return cancelled();
    for (std::size_t axis = 0; axis < 2; ++axis) if (!assessReversals(series, result, axis, cancel)) return cancelled();
    for (std::size_t a = 0; a < 2; ++a) for (std::size_t b = a + 1; b < 3; ++b)
        if (!assessPair(series, result, a, b, history, cancel)) return cancelled();
    if (!assessLongDurationScope(series, result, cancel)) return cancelled();
    finish(result);
    return result;
}

AccelerationAssessment assessAccelerationF2291_25(const AccelerationSeries& series, const std::function<bool()>& cancel) {
    AccelerationAssessment result;
    if (!validate(series, result, cancel)) return result;
    std::array<std::vector<double>, 3> processed;
    for (std::size_t axis = 0; axis < 3; ++axis) if (!filter(axes(series)[axis], series.stepSeconds, processed[axis], cancel)) {
        result.cancelled = true;
        return result;
    }
    return assessProcessedAccelerationF2291_25({processed[0], processed[1], processed[2], series.stepSeconds,
        series.startTimeSeconds, series.seatIndex}, cancel);
}
