...
**Checkpoint2 working tree,2026-09-22: `2.0.0-rift.1` is implemented and being verified; it is not packaged or promoted yet. Play still runs the verified foundation checkpoint below.**

- Opening rise is now160m: the seed42 measured descending drop is155.012m, maximum pitch56.46°, bank38.40°, heading change56.51°, and crest-to-low-recovery7.22s. This better expresses the FF rounded crest / twisted descent / low recovery composition. FF's published opening is smaller; exact equivalence is unclaimed.
- The low return now heads directly toward a broad station turn. This removes the redundant outward waypoint and reduces the baseline circuit to8533.921m /197.858s. The Immelmann and descending ravine roll remain. Continuous negative-G, native force-rate, history, clearance and convergence gates pass.
- Terrain now has an exposed outbank bay, broad cliff-foot/camelback benches, connected regional foothills and a return valley. Eight foothills and four ravine cuts use the shared8m surface. The latest terrain still needs packaged GPU visual review.
- All eight representative cases pass in `native/build/evidence/rift-corpus-opening160`: baseline42, flat42, trims-off42, seed5 and lower42 accept candidate0; seed77, flowing7 and higher314 accept candidate2. The310km/h case needed only a small extra cut from the camelback's final positive hold; its protected crest/body remain unchanged.
- Full baseline operating checks pass for trims off, fully deployed and20% lower drag, at960/1920Hz, in `rift-operating-checks.log`. Lower-drag Gz−1.50826 is a0.551% nominal peak exceedance, reported separately; ASTM and20g/s gates pass. Nominal baseline Gz−1.42290…4.11950, |Gy|0.68301, |Gx|4.29630; full swept ground gap2.37915m.
- Nearly level spline corridors now eliminate redundant pitch variables while preserving their literal endpoint height/jets. A nanometre-height regression passes. Sparse solver work, cancellation inside placement, bounded true-arc caching, direct authored-frame evaluation, parallel spatial replay, repeated-ground-certificate reuse and faster serialization improve loading/generation without trusting saved checksums.
- Latest generation observation10.584s and fresh saved validation3.214s after overlapping the independent replays with clearance. Controlled five-run packaged benchmarks against matched Escarpment and0.9 are still required; do not mark performance accepted yet.
- Role-specific fixed metadata is omitted from editable recipes and silent no-op edits are rejected. Legacy recipe fields still load at their original fixed values. Brake capacity is now wired to its recipe parameter. The complete CLI edit/reorder/save/reload/re-edit demonstration passed in `native/build/evidence/recipe-final`; both edited rides and the freshly validated saved ride were accepted.
- Native suites passed after correcting the early motor-save preflight and updating timing/arc-cache/support-family assertions; the focused CLI argument tests also pass. `native/build/*-tests.xml`, `corrected-checks.xml` and `final-integration-tests.xml` retain results. Six Unreal contracts and the editor build passed via `Saved/BuildRuns/20260921-171015-599`. Final package/GPU/CI for this checkpoint are pending.
- Loading now has a dedicated phase/elapsed/progress panel. The runtime verification driver additionally probes generation, mesh preparation and scene cancellation while the old ride keeps playing. These new GPU probes have compiled but have not run in a package yet.

Finish checkpoint2 with the reviewed commit/push and full checkpoint CI, clean versioned package, front/rear GPU checks and exact shortcut promotion. Then finish checkpoint3 with controlled performance, final representative verification, remaining cleanup and another verified package. Keep the single compiler worker and single-agent restriction.

### Shipped foundation checkpoint: historical measured snapshot

The foundation seed42 baseline passed native acceptance: continuous all-seat F2291-25 and project caps, C3 geometry/orientation, retained-source replay, finite-train energy, complete swept clearance, temporal convergence and independent spatial refinement. **Checkpoint1 is delivered as2.0.0-foundation.1. Code commit39cdba3 passed focused GitHub CI, packaging and actual GPU front/rear traversal. Play targets that exact verified executable. Final generator acceptance remains incomplete.** See `docs/checkpoints/05-foundation.json` and `05-play-target.json`.
