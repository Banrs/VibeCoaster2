...
Start with [HANDOFF.md](HANDOFF.md) for current status, build commands, and unresolved issues. `dist/current.json` identifies the restored Default 1 package and its isolated saved-ride profile. Double-click `Play VibeCoaster2.lnk` in the repository or `VibeCoaster2` on the desktop to launch that packaged game directly. Recreate the verified shortcuts with `native/unreal/scripts/create_shortcuts.ps1 -Desktop`. The newer generator remains under development until a reviewed package is promoted.
