...
Standalone C++20 hybrid FVD/spline coaster generator and Unreal 5.8 viewer. The active design and acceptance contract is [NEXT_SESSION.md](NEXT_SESSION.md). The revised-ride checkpoint, **2.0.0-rift.1**, passed native acceptance, the eight-case corpus, CI and packaged front/rear GPU traversal. Controlled performance and final generator verification remain.

Open **Play VibeCoaster2.lnk** for Rift; **F9** loads its saved seed42 ride and **Space** starts or pauses it. **Enter** generates, **Tab** opens setup, **1/2/3** select front/middle/rear, **M** toggles overview, **R** restarts, and **F5** saves. It uses `UserData-Rift-Verified`. Separate **Foundation**, **Escarpment** and **Highlands** shortcuts preserve earlier packages/profiles. The original sibling project remains unchanged. [Checkpoint evidence](docs/checkpoints/06-rift.json) and [exact Play identity](docs/checkpoints/06-play-target.json) identify tested code commit `b311cd1`.
